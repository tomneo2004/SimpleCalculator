...
class SubtractNode: OperatorNode {
    ...
    override func evaluate() -> NumberNode? {
        ...
        guard let leftHandNode = self.parentNode else{
            fatalError("SubstractNode must have parent node")
        }
        ...
        guard let leftNumber = leftHandNode as? NumberNode else{
            fatalError("SubstractNode left hand/parent node is not a NumberNode")
        }
        
        guard let rightNumber = rightHandNode as? NumberNode else{
            fatalError("SubstractNode right hand/child node is not a NumberNode")
        }
