...
class DivisionNode: OperatorNode {
    ...
    override func evaluate() -> NumberNode? {
        ...
        guard let leftHandNode = self.parentNode else{
            fatalError("DivisionNode must have parent node")
        }
        ...
        guard let leftNumber = leftHandNode as? NumberNode else{
            fatalError("DivisionNode left hand/parent node is not a NumberNode")
        }
        
        guard let rightNumber = rightHandNode as? NumberNode else{
            fatalError("DivisionNode right hand/child node is not a NumberNode")
        }
